...
import com.utmaximur.alcoholtracker.data.room.AlcoholTrackDao
import com.utmaximur.alcoholtracker.data.room.DrinkDao
import java.util.*
...
abstract class StorageService: RoomDatabase() {
    abstract fun drinkDao(): DrinkDao
    abstract fun trackDao(): AlcoholTrackDao

//    var room = AppDatabase.getAppDataBase(context)

    companion object {
        var INSTANCE: StorageService? = null

        fun getAppDataBase(context: Context): StorageService? {
            if (INSTANCE == null){
...
//                        .allowMainThreadQueries()
                        .addCallback(object : Callback() {
                            override fun onCreate(db: SupportSQLiteDatabase) {
                                super.onCreate(db)
                                ioThread {
                                    initRealmWithData(context).forEach {
                                        INSTANCE?.drinkDao()?.addDrink(it)!!
                                    }
                                }
                            }
                        })
